...
bool UserManager::create(User &user, const std::string &password)
{
    try
    {
        user.password_hash = utils_sg::PasswordHasher::generate_hash(password);
    }
...
bool UserManager::authenticate(const std::string &username, const std::string &password)
{
    ...
    const bool valid = utils_sg::PasswordHasher::validate_password(password, hash);
...
std::vector<User> UserManager::get_all()
{
    ...
    const std::string sql = "SELECT user_id, username, role, created_at FROM users";
